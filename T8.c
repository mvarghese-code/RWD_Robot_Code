...
int main()
{
	...
	int esc_ch[5] = {1,2,3,4,5}; //row = PWM channel, col = commanded output
	//int esc_ch[5][5] = {1,2,3,4,5},{0,0,0,0,0}; //row = PWM channel, col = commanded output
...
// 40ms Tasks ============================================================================
				if (curms-prev40ms >= 40) {
					...
					if (datamain[0]<1450 && datamain[1]<1450){//Reverse
						rc_servo_send_pulse_normalized(esc_ch[0],-1);
						rc_servo_send_pulse_normalized(esc_ch[1],-1);
					}

					if(datamain[0]>1550 && datamain[1]>1550 ){//Forward
						rc_servo_send_pulse_normalized(esc_ch[0],1);
						rc_servo_send_pulse_normalized(esc_ch[1],1);
					}
